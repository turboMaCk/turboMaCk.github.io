...
a functions with many arguments. For these records can work as substitution for named arguments (which OCaml has but Haskell does not).
Using record with named fields brings additional semantic clarity, making it easier to understand the API.
For instance with arguments for networking functions.
...
It's common to define non empty list type as ~type NonEmpty a = Cons a (List a)~
and this several libraries that does this already exist.
This is also quite close to definition of [[https://hackage.haskell.org/package/base-4.14.0.0/docs/Data-List-NonEmpty.html#g:1][Data.List.NonEmpty]] in Haskell's base.

I think synonym definition is especially good suited in Elm because:

1. Elm doesn't have [[https://en.wikipedia.org/wiki/Ad_hoc_polymorphism][ad hoc polymorphism]]. Unlike in Haskell and PureScript we can't make this data type member of class anyway.
2. This definition doesn't come in base and therefore is hard to reuse in libraries - it would lock library users to some specific definition.
3. Using alias doesn't require using library in order to consume the type - it's easy to bring it in later.
4. ~elm-community/list-extra~ already provides ~uncons~ function which is a compatible constructor with this definition.

That's not to say there won't be any benefits in having similar definition in language other than Elm.
[[https://twitter.com/fried_brice/status/1232773993507147776][Some folks might pefer it]]. I think in case of Elm the case is even stronger.
...
Zipper type contains private data field users of the library are not suppose to be
exposed to and mess with. This type has to maintain internal invariant on its data
and therefore opaque type is a right choice in this case.

Both ~NonEmpty~ and ~Zipper~ comes with full featured implementations including but not limited to
[[https://en.wikipedia.org/wiki/Functor][Fucntor]] [[https://en.wikipedia.org/wiki/Applicative_functor][Applicative Functor]] an [[https://en.wikipedia.org/wiki/Monad_(category_theory)][Comonad]] implementations.
...
Anonymous types do change the ergonomics of a language.
We should always think about what is an appropriate level of openness or closeness
when designing APIs.
